<template>
  ...
  >
    <FilterSeclectPlaceholder class="w-full" :label="value.length ? `${value.length} letters` : 'Letters'" />
    <template #dropdown>
  ...
  <div v-else class="w-full">
    <FilterSeclectPlaceholder class="w-full" :label="value.length ? `${value.length} letters` : 'Letters'" @click="handleOpenSwipeBar" />
    <SwipeBar
      label="Contain letters"
...
const handleChange = (letter: string) => {
  if (selectedLetters.value.indexOf(letter) === -1) {
    selectedLetters.value.push(letter);
  } else {
    selectedLetters.value = selectedLetters.value.filter((val) => val !== letter);
  }
};
